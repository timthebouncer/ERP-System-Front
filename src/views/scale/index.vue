  ...
  <div>
    <v-container fluid class="wrapper">
      ...
      <div class="mid d-flex pa-2">
        <div class="weight-wrap">
          <div class="weight-scale">
            ...
            <div v-else-if="tkgStatus">
              {{ Math.floor(changeValue) }}斤{{ twTael }}兩
            </div>
        ...
        <div class="pl-5">
          <v-form ref="form" v-model="valid" lazy-validation>
            <v-row align="center" style="margin-top: -32px;">
              ...
              <v-col class="d-flex pb-0" cols="12" sm="6" md="6">
                <v-text-field
                  v-model="addOrderForm.carcassWeight"
                  disabled
                  label="屠體重量(公斤) :"
                  style="font-size: 22px;"
                />
        ...
        <div class="mid-btn mx-5 mt-1">
          ...
          >
          <v-btn
            :disabled="inboundPrintStatus"
            class="print mt-9"
            @click="inboundPrint"
            ><h1 style="font-size: 36px;">入庫列印</h1></v-btn
          >
        </div>
      </div>
      ...
      <v-snackbar
        ...
        :top="'top'"
        :color="inboundMsg ? 'error' : 'success'"
        :timeout="2500"
      >
    ...
    <AccumulateDialog
      ...
      @getZero="getZero"
    />
    <IpDialog :show="ipShow" @close="closeIpDialog" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeDepot() {
      ...
      localStorage.setItem("depot", this.productDepot);
    },
    accumulate() {
      //防止累加按鈕連續操作
    ...
    changeUnit(value) {
      if (value === "kg") {
        return (
          ...
          (this.changeValue =
            this.deductionValue && this.displayValue !== 0
        ...
        return (
          (this.changeValue =
            this.deductionValue && this.displayValue !== 0
              ? (this.displayValue - this.deductionValue) / 0.6
              : this.displayValue / 0.6),
          (this.kgStatus = false),
          (this.gStatus = false),
          (this.tkgStatus = true)
        );
        ...
        return (
          (this.changeValue =
            this.deductionValue && this.displayValue !== 0
              ? this.displayValue * 1000 - this.deductionValue * 1000
    ...
    updateStock() {
      ...
      this.$scale.Inventory.updateStock({
        ...
        id: this.inventoryId,
        //如果有動態條碼就使用，反之則拿固定條碼
        barcode: this.barcodeStorage
          ? this.barcodeStorage
          : this.stockInForm.barcode,
        //目前庫存 - 要取消的數量
    ...
    changeBarcode() {
      ...
      return new Promise(async resolve => {
        ...
        if (this.count > 1) {
          let status = true;
          for (
            let i = this.commodityNumber;
            i < Number(this.count) + Number(this.commodityNumber);
            i++
          ) {
            ...
            if (status) {
              ...
              return (
                (this.inboundStatus = true), (this.inboundMsg = "列印發生錯誤")
              );
            }
          }
        } else {
